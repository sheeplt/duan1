                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        if (isset($_REQUEST["btnsubmit"])) {
                            ...
                            $table = $p->dshoadon1GV($search);
                        } else {
                            $table = $p->dshoadonGV();
                        }
                        if ($table) {
                            if (mysqli_num_rows($table) > 0) {
                                echo "<h1 style='text-align:center;'>DANH SÁCH HÓA ĐƠN GIÁO VIÊN</h1>";
                        ?>
                                <br>
                                <form class="js-form-submit-data" action="#" method="POST" enctype="multipart/form-data">
                                    <span class="icon"><i class="fa fa-search"></i></span>
                                    <input type="search" name="search" id="search" placeholder="Nhập mã giáo viên" />
                                ...
                                echo "<td style='color: white; text-align:center;border: 1px solid black;  background: #015f95;'>Tên giáo viên</td>";
                                ...
                                while ($row = mysqli_fetch_assoc($table)) {
                                    ...
                                    echo "<td style='border: 1px solid black; background: white;'>";
                                ?>
                                    <form method="POST" action="QL.php?cthoadongiaovien=<?php echo $row["MaGV"] ?>&mahd=<?php echo $row["MaHoaDon"]; ?>">
                                        <?php
                                        $today = time();
                                        $thang = date("m", $today);
                                        $nam = date("Y", $today);
                                        //ngay thanh toán
                                        $ngaytt = strtotime($row["NgayTT"]);
                                        $thangtt = date("m", $ngaytt);
                                        $namtt = date("Y", $ngaytt);
                                        if ($row["TrangThaiHoaDon"] == 0) {
                                        ?> <input name="chitiet" type="submit" value="Chi tiết" style="background-color: white; border: solid white; color: blue;">
                                        <?php
                                        } elseif ($row["TrangThaiHoaDon"] == 1) {
                                            echo "Đã thanh toán";
                                        }
                        ...
                        if (isset($_REQUEST["btnsubmit"])) {
                            ?> <br>
                            <form class="js-form-submit-data" action="QL.php?hoadongiaovien" method="POST" enctype="multipart/form-data">
